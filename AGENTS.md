...
- Implemented component inventory (40 files; keep in sync with `src/components/`):
  ...
  - `auth/GoogleLoginButton` — signs into the in-memory mock preview or starts backend Google OAuth in API mode. OAuth return routes to the first unfinished onboarding step. `wallet/WalletProvider` + `wallet/WalletConnectButton` connect an Injective EVM wallet and, in API mode, complete the backend challenge + EIP-191 signature verification flow.
...
- Data: typed registries and contracts in `src/lib/` (`types.ts` is the shared public-data contract; `contracts/` defines frontend boundaries; `mocks/` supplies deterministic fixtures and the mock API client; `api/` supplies backend auth/account/onboarding adapters; `bounties`/`notices`/`members`/`hall-of-fame`/`landing`/`signup`; `admin.ts` = keyed admin adapters/overlays). Public profile routes use fixtures in mock mode and `/users/:nickname` in API mode; unknown profiles call `notFound()`.
